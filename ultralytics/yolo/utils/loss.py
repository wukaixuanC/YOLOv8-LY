...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max, use_dfl=False):
        ...
        if self.use_wiseiou:
            self.wiou_loss = WiseIouLoss(ltype='ShapeIoU', monotonous=False, inner_iou=True)
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask,
        ...
        if self.use_wiseiou:
            ...
            # wiou = self.wiou_loss(pred_bboxes[fg_mask], target_bboxes[fg_mask], ret_iou=False, ratio=0.7, **{'mpdiou_hw':mpdiou_hw}).unsqueeze(-1) # Wise-MPDIoU,Wise-Inner-MPDIoU
