...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Detect_DyHead, Detect_AFPN_P2345, Detect_AFPN_P2345_Custom, Detect_AFPN_P345, Detect_AFPN_P345_Custom, 
                          Detect_Efficient, DetectAux, Detect_DyHeadWithDCNV3, Segment)):
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 ...
                 C2f_Faster_AMA, C2f_DBB, GSConv, VoVGSCSP, VoVGSCSPC, C2f_CloAtt, C3_CloAtt, SCConv, C2f_SCConv, C3_SCConv, C2f_ScConv, C3_ScConv,
                 C3_EMSC, C3_EMSCP, C2f_EMSC, C2f_EMSCP, RCSOSA, KWConv, C2f_KW, C3_KW, DySnakeConv, C2f_DySnakeConv, C3_DySnakeConv,
                 DCNv2, C3_DCNv2, C2f_DCNv2, DCNV3_YOLO, C3_DCNv3, C2f_DCNv3, C3_Faster, C3_ODConv,
                 ...
                 C3_DLKA, C2f_DLKA, CSPStage, SPDConv):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3, C2f_Faster, C2f_ODConv, C2f_Faster_AMA, C2f_DBB,
                     ...
                     C3_DCNv2, C2f_DCNv2, C3_DCNv3, C2f_DCNv3, C3_Faster, C3_ODConv, C3_OREPA, C2f_OREPA, C3_DBB,
                     C3_REPVGGOREPA, C2f_REPVGGOREPA, C3_DCNv2_Dynamic, C2f_DCNv2_Dynamic, C3_ContextGuided, C2f_ContextGuided, 
                     C3_MSBlock, C2f_MSBlock, C3_DLKA, C2f_DLKA, CSPStage):
            ...
            m = m(*args)
            c2 = m.channel
        elif m in {AMA, SpatialAttention, BiLevelRoutingAttention, BiLevelRoutingAttention_nchw,
            ...
            args = [c2, *args]
            # print(args)
        elif m in {DenseBlockAMA}:
            # print('ch[f] = ', ch[f])
            num_input_features = ch[f]
            # growth_rate = int(3/args[0]) * ch[f]
            growth_rate = 32
            c2 = num_input_features + args[0] * growth_rate
            # print('c2.1 = ', c2)
            args = [num_input_features, growth_rate, *args]
        elif m in {TransitionAMA}:
            num_input_features = c2
            # print('c2.2 = ', c2)
            # num_output_features = args[0]
            num_output_features = int((0.5) * num_input_features)
            c2 = num_output_features
            args = [num_input_features, num_output_features]
