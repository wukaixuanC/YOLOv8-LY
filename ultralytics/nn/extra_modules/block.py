...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C2f_Faster', 'C3_Faster', 'C3_ODConv', 'C2f_ODConv',
           'Partial_conv3', 'C2f_DBB',
           'GSConv', 'VoVGSCSP', 'VoVGSCSPC', 'C2f_CloAtt', 'C3_CloAtt', 'SCConv', 'C3_SCConv', 'C2f_SCConv', 'ScConv',
           'C3_ScConv', 'C2f_ScConv',
           ...
           'FocalModulation',
           'C3_OREPA', 'C2f_OREPA', 'C3_DBB', 'C3_REPVGGOREPA', 'C2f_REPVGGOREPA', 'C3_DCNv2_Dynamic',
           'C2f_DCNv2_Dynamic',
           'SimFusion_3in', 'SimFusion_4in', 'IFM', 'InjectionMultiSum_Auto_pool', 'PyramidPoolAgg', 'AdvPoolFusion',
           'TopBasicLayer',
           'C3_ContextGuided', 'C2f_ContextGuided', 'C3_MSBlock', 'C2f_MSBlock', 'ContextGuidedBlock_Down', 'C3_DLKA',
           'C2f_DLKA', 'CSPStage', 'SPDConv',
           'BiFusion', 'AMA', 'DenseBlockAMA', 'TransitionAMA',
           'C2f_Faster_AMA']
...
class Bottleneck_KW(Bottleneck):
    ...
    def __init__(self, c1, c2, wm=None, wm_name=None, shortcut=True, g=1, k=(3, 3),
...
class C2f_KW(C2f):
    def __init__(self, c1, c2, n=1, wm=None, wm_name=None, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(
            Bottleneck_KW(self.c, self.c, wm, wm_name, shortcut, g, k=(3, 3), e=1.0) for _ in range(n))
...
class DenseLayerAMA(nn.Sequential):
    """Basic unit of DenseBlock (using bottleneck layer) """

    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        super(DenseLayerAMA, self).__init__()
        ...
        self.relu2 = nn.ReLU()
        self.h_swish = h_swish(inplace=True)
        self.AMA = AMA(bn_size * growth_rate)
...
class DenseBlockAMA(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, num_layers=3, bn_size=1, drop_rate=0):
        super(DenseBlockAMA, self).__init__()
        for i in range(num_layers):
            if i == 0:
                # print('num_input_features = ', num_input_features)
                # print('growth_rate = ', growth_rate)
                # print('num_input_features + i * growth_rate = ', num_input_features + i * growth_rate)
                # print('i = ', i)

                self.layer = nn.Sequential(
                    DenseLayerAMA(num_input_features + i * growth_rate, growth_rate, bn_size, drop_rate)
                )
            else:
                # print('num_input_features2 = ' , num_input_features)
                # print('growth_rate2 = ', growth_rate)
                # print('num_input_features + i * growth_rate = ', num_input_features + i * growth_rate)
                # print('i2 = ', i)
                layer = DenseLayerAMA(num_input_features + i * growth_rate, growth_rate, bn_size, drop_rate)
                self.layer.add_module("denselayer%d" % (i + 1), layer)

    def forward(self, input):
        # print('input =', input.size())
        # print('self.layer(input) = ', self.layer(input).size())
        return self.layer(input)


class TransitionAMA(nn.Sequential):
    def __init__(self, num_input_feature, num_output_features):
        super(TransitionAMA, self).__init__()
        self.bn = nn.BatchNorm2d(num_input_feature)
        self.relu = nn.ReLU()
        self.h_swish = h_swish(inplace=True)
        self.AMA = AMA(num_input_feature)
...
class Faster_Block_AMA(nn.Module):
    def __init__(self,
                 inc,
                 dim,
                 n_div=4,
...
class C2f_Faster_AMA(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(Faster_Block_AMA(self.c, self.c) for _ in range(n))